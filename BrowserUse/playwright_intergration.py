...
class PlaywrightScreenshotAction(BaseModel):
    ...
    quality: int | None = Field(
        default=None, description="JPEG quality (1-100), only for .jpg/.jpeg files"
    )
...
class PlaywrightGetTextAction(BaseModel):
    ...
    selector: str = Field(
        ..., description='CSS selector to get text from. Use "title" for page title.'
    )
...
async def playwright_fill_form(params: PlaywrightFillFormAction, browser_session: BrowserSession):
    ...
    try:
        ...
        return ActionResult(
            ...
            long_term_memory=f"Filled form with: {form_data}",
        )
...
@tools.registry.action(
    "Extract text from elements using Playwright's powerful CSS selectors and XPath support.",
